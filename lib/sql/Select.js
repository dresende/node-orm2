...
Builder.prototype.build = function () {
	...
	// tables
	lst = [];
	var this_opts_table_length = this.opts.table.length;
	for (i = 0; i < this_opts_table_length; i++) {
	...
	if (this.opts.merge) {
		var this_opts_merge_length = this.opts.merge.length;
		for (i = 0; i < this_opts_merge_length; i++) {
	...
	// where
	lst = [];
	var this_opts_where_length = this.opts.where.length;
	for (i = 0; i < this_opts_where_length; i++) {
	...
	if (this.opts.hasOwnProperty("order")) {
		if (Array.isArray(this.opts.order)) {
			...
			var this_opts_order_length = this.opts.order.length;
			for (i = 0; i < this_opts_order_length; i++) {
