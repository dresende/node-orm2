...
function Model(opts) {
	...
	model.drop = function (cb) {
		...
		return cb(ErrorCodes.generateError(ErrorCodes.NO_SUPPORT, "Driver does not support Model.drop()", { model: this }));
	};
	...
	model.sync = function (cb) {
		...
		return cb(ErrorCodes.generateError(ErrorCodes.NO_SUPPORT, "Driver does not support Model.sync()", { model: this }));
	};
	...
	model.get = function () {
		...
		if (typeof cb !== "function") {
		    throw ErrorCodes.generateError(ErrorCodes.MISSING_CALLBACK, "Missing Model.get() callback", { model: this });
		}
		...
		if (ids.length !== opts.id.length) {
		    throw ErrorCodes.generateError(ErrorCodes.PARAM_MISSMATCH, "Model.get() IDs number missmatch (" + opts.id.length + " needed, " + ids.length + " passed)", { model: this });
		}
		...
		opts.driver.find(model_fields, opts.table, conditions, { limit: 1 }, function (err, data) {
			...
			if (data.length === 0) {
			    return cb(ErrorCodes.generateError(ErrorCodes.NOT_FOUND, "Not found", { model: this }));
			}
	...
	model.one = function () {
		...
		if (cb === null) {
		    throw ErrorCodes.generateError(ErrorCodes.MISSING_CALLBACK, "Missing Model.one() callback", { model: this });
		}
	...
	model.count = function () {
		...
		if (typeof cb !== "function") {
		    throw ErrorCodes.generateError(ErrorCodes.MISSING_CALLBACK, "Missing Model.count() callback", { model: this });
		}
	...
	model.exists = function () {
		...
		if (typeof cb !== "function") {
		    throw ErrorCodes.generateError(ErrorCodes.MISSING_CALLBACK, "Missing Model.exists() callback", { model: this });
		}
