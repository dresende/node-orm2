	...
	for (k in opts.properties) {
		switch (opts.properties[k].type) {
			case "text":
				...
				break;
			case "number":
				if (opts.properties[k].rational === false) {
			...
			case "date":
				...
				break;
			case "binary":
				if (opts.properties[k].big === true) {
					definitions.push(driver.escapeId(k) + " LONGBLOB");
				} else {
		...
		if (opts.properties[k].hasOwnProperty("defaultValue")) {
			definitions[definitions.length - 1] += " DEFAULT " + driver.db.escape(opts.properties[k].defaultValue);
		}
	}
