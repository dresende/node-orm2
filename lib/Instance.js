...
function Instance(opts) {
	...
	var normalizedValidations = function() {
		var list = [];
		for (var k in opts.validations) {
			if (Array.isArray(opts.validations[k])) {
				for (var i = 0; i < opts.validations[k].length; i++) {
					list.push([ k, opts.validations[k][i] ]);
				}
			} else {
				list.push([ k, opts.validations[k] ]);
			}
		}
		return list;
	};
	var enumerateValidationErrors = function (cb) {
		var validations = normalizedValidations();

		var runValidation = function (errors, validation, cb) {
			validation[1](instance[validation[0]], function (msg) {
				if (msg) {
					errors[validation[0]] = errors[validation[0]] || [];
					errors[validation[0]].push(msg);
				}
				cb(null, errors);
			}, instance, opts.model, validation[0]);
		}
		async.reduce(validations, {}, runValidation, function(err, errors) {
			if (err) {
				cb(err);
			} else {
	...
	var firstValidationError = function (cb) {
		var pending = normalizedValidations();
	...
	var handleValidations = function (cb) {
		if (opts.returnAllErrors) {
			enumerateValidationErrors(cb);
		} else {
			firstValidationError(cb);
		}
	};
	...
	Object.defineProperty(instance, "getValidationErrors", {
		value: function(cb) {
			enumerateValidationErrors(cb);
