		...
		Hook.wait(instance, opts.hooks.beforeValidation, function (err) {
			...
			for (k in opts.validations) {
				...
				if (Model.properties[k]) {
					required = Model.properties[k].alwaysValidate || Model.properties[k].required;
				} else {
					for (i = 0; i < opts.one_associations.length; i++) {
                                                for (a in opts.one_associations[i].field) {
                                                    if (a === k) {
                                                            required = opts.one_associations[i].alwaysValidate || opts.one_associations[i].required;
                                                            break;
                                                    }
                                                }
					}
