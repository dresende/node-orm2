...
Driver.prototype.insert = function (table, data, cb) {
    ...
    var db = this.db;
    console.log(this.QueryInsert.build());
	this.db.query(this.QueryInsert.build() + " RETURNING *", function (err, result) {
		if (err) {
			return cb(err);
		}
        db.query("SELECT pg_attribute.attname FROM pg_index, pg_class, pg_attribute WHERE pg_class.oid = '"+table+"'::regclass AND indrelid = pg_class.oid AND pg_attribute.attrelid = pg_class.oid AND  pg_attribute.attnum = any(pg_index.indkey) AND indisprimary",function(err,pk_name){
            if (err) {
                return cb(err);
            }
            console.log('Insert id: ' + result.rows[0][pk_name.rows[0].attname]);
            return cb(null, {
                id: result.rows[0][pk_name.rows[0].attname] || null
            });
        });
	});
};
...
Driver.prototype.update = function (table, changes, conditions, cb) {
    ...
    console.log(this.QueryUpdate.build());
