...
function Instance(Model, opts) {
	...
	var setInstanceProperty = function (key, value, initialization) {
		// @mod default value for initialization parameter
		initialization = initialization || false;

		var prop = Model.allProperties[key] || opts.extra[key];

		if (prop) {
			// @mod if the following criteria match, this property should not be modified:
			// - this model has been persisted (i.e. not new)
			// - this property is readonly
			// - initialization flag is false
			if (!opts.is_new && prop.readonly === true && initialization === false) {
				return false;
			}
	...
	var addInstanceProperty = function (key) {
		...
		// @mod pass 'true' as we are initializing this model
		setInstanceProperty(key, defaultValue, true);
	...
	Object.defineProperty(instance, "save", {
		value: function () {
			...
			for (var k in data) {
				// @mod check if model has property before setting new value
				// this is ok for relational DB, might want to modify for document DB
				if (this.hasOwnProperty(k) && data.hasOwnProperty(k)) {
					this[k] = data[k];
				}
			}
