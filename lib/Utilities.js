...
exports.wrapFieldObject = function (obj, model, altName, alternatives) {
	if (!obj) {
		obj = model.settings.get("properties.association_key")
		                    .replace("{name}", altName.toLowerCase())
		                    .replace("{field}", "id");
	}
...
exports.formatField = function (model, name, required, reversed) {
	...
	for (var i = 0; i < keys.length; i++) {
		field_name = reversed ? keys[i] : model.settings.get("properties.association_key")
