...
exports.syncConstraints = function (driver, opts, cb) {
	var queries = [];
	var k, i, pending;
	...
	for (i = 0; i < opts.one_associations.length; i++) {
		...
		for (k in opts.one_associations[i].field) {
                    queries.push(
                            "ALTER TABLE " + driver.query.escapeId(opts.table) + " " +
                            "DROP FOREIGN KEY fk_" + k + "_" + opts.one_associations[i].name
                    );
                    queries.push(
                            "ALTER TABLE " + driver.query.escapeId(opts.table) + " " +
                            "ADD CONSTRAINT fk_" + k + "_" + opts.one_associations[i].model.table + " FOREIGN KEY (" + driver.query.escapeId(k) + ") " +
                            "REFERENCES " + opts.one_associations[i].name + "(`id`) " + 
                            "ON UPDATE " + (opts.one_associations[i].onUpdate ? opts.one_associations[i].onUpdate.toUpperCase() : 'CASCADE') + " " + 
                            "ON DELETE " + (opts.one_associations[i].onDelete ? opts.one_associations[i].onDelete.toUpperCase() : 'RESTRICT')
                    );
		}
	}
	...
	for (i = 0; i < queries.length; i++) {
		driver.execQuery(queries[i], function (err) {
			if (--pending === 0) {
				return cb(err && err.code === 'ER_CANT_CREATE_TABLE' ? null : err);
			}
		});
	}
};
