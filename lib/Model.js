...
function Model(opts) {
	...
	model.syncConstraints = function (cb) {
		if (arguments.length === 0) {
			cb = function () {};
		}
		if (typeof opts.driver.syncConstraints === "function") {
			try {
				opts.driver.syncConstraints({
					extension           : opts.extension,
					id                  : opts.id,
					table               : opts.table,
					properties          : opts.properties,
		...
		return cb(ErrorCodes.generateError(ErrorCodes.NO_SUPPORT, "Driver does not support Model.syncConstraint()", { model: opts.table }));
	};
