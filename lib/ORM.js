...
function ORM(driver_name, driver, settings) {
	...
	var onError = function (err) {
		if (this.settings.get("connection.reconnect")) {
			...
			this.driver.reconnect(function (error) {
				// if failed try reconnect again after timeout
				if (error && this.settings.get("connection.reconnectTime")) {
					setTimeout(function() {
						onError(error);
					}, this.settings.get("connection.reconnectTime"));
				} else {
