...
var ACCESSOR_METHODS = ["hasAccessor", "getAccessor", "setAccessor", "delAccessor", "addAccessor"];
...
exports.prepare = function (db, Model, associations) {
  Model.hasMany = function () {
    ...
    var association = {
      ...
      getAccessorAsync : (opts.getAccessor || ("get" + assocTemplateName)) + promiseFunctionPostfix,
      setAccessorAsync : (opts.setAccessor || ("set" + assocTemplateName)) + promiseFunctionPostfix,
      hasAccessorAsync : (opts.hasAccessor || ("has" + assocTemplateName)) + promiseFunctionPostfix,
      delAccessorAsync : (opts.delAccessor || ("remove" + assocTemplateName)) + promiseFunctionPostfix,
      addAccessorAsync : (opts.addAccessor || ("add" + assocTemplateName)) + promiseFunctionPostfix
    };
...
function extendInstance(Model, Instance, Driver, association, opts, createInstance) {
  ...
  for (var y = 0; y < ACCESSOR_METHODS.length; y++) {
    var accessorMethodName = ACCESSOR_METHODS[y];
    Object.defineProperty(Instance, association[accessorMethodName] + promiseFunctionPostfix, {
      value: Promise.promisify(Instance[association[accessorMethodName]]),
      enumerable: false,
      writable: true
    });
  }
