...
exports.prepare = function (Model, associations) {
	Model.hasMany = function () {
		...
		associations.push({
			...
			field          : opts.field || Settings.get("properties.association_key").replace("{name}", name),
			mergeTable     : opts.mergeTable || (Model.table + "_" + name),
			mergeId        : opts.mergeId || Settings.get("properties.association_key").replace("{name}", Model.table),
			mergeAssocId   : opts.mergeAssocId || Settings.get("properties.association_key").replace("{name}", name),
