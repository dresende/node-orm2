...
function Instance(opts) {
	...
	var handleValidations = function (cb) {
		var pending = [];
		var opts_validation_length = 0;
		for (var k in opts.validations) {
			if (Array.isArray(opts.validations[k])) {
				opts_validation_length = opts.validations[k].length;
				for (var i = 0; i < opts_validation_length; i++) {
	...
	var saveInstance = function (cb) {
		...
		handleValidations(function (err) {
			...
			for (var k in opts.data) {
				...
				if (opts.properties[k]) {
					...
					if (opts.driver.valueToProperty) {
						data[k] = opts.driver.valueToProperty(data[k], opts.properties[k]);
					}
				} else {
					data[k] = opts.data[k];
				}
			...
			if (opts.is_new || !data.hasOwnProperty(opts.id)) {
				...
				var opts_changes_length = opts.changes.length;
				for (var i = 0; i < opts_changes_length; i++) {
	...
	var saveInstanceExtra = function (cb) {
		...
		var opts_extrachanges_length = opts_extrachanges_length.extrachanges.length;
		for (var i = 0; i < opts_extrachanges_length; i++) {
	...
	var saveInstanceProperty = function (key, value) {
		...
		if (opts.properties[key]) {
			changes[key] = Property.validate(changes[key], opts.properties[key]);
			 if (opts.driver.valueToProperty) {
				changes[key] = opts.driver.valueToProperty(changes[key], opts.properties[key]);
			}
		}
