...
exports.prepare = function (db, Model, associations) {
  Model.extendsTo = function (name, properties, opts) { // TODO: extendsTo and find methods should be changed to Async
    opts = opts || {};
...
exports.autoFetchAsync = function (Instance, associations, opts) {
  return new Promise (function (resolve, reject) {
    ...
    associations.forEach(function (association) {
      autoFetchInstanceAsync(Instance, association, opts, autoFetchDone);
    });
  });
};
...
function extendInstance(Model, Instance, Driver, association, opts) {
  ...
  Object.defineProperty(Instance, association.hasAccessor + 'Async', {
    value : function () {
      return new Promise (function(resolve, reject) {
        if (!Instance[Model.id]) {
          return reject(new ORMError("Instance not saved, cannot get extension", 'NOT_DEFINED', { model: Model.table }));
        } else {
          association.model.get(util.values(Instance, Model.id), function (err, extension) {
            if (err) return reject(err);
            return resolve(!err && extension ? true : false);
          });
        }
      });
    },
  ...
  Object.defineProperty(Instance, association.getAccessor + 'Async', {
    value: function (opts) {
      return new Promise (function(resolve, reject) {

        if (typeof opts == "function") {
          var callbackData = opts;
          opts = {};
        }
        ...
        if (!Instance[Model.id]) {
          ...
          association.model.get(util.values(Instance, Model.id), opts, function(err, result) {
            if (err) return reject(err);
            if (callbackData) return resolve(callbackData);
  ...
  Object.defineProperty(Instance, association.setAccessor + 'Async', {
    value : function (Extension) {
      return new Promise (function(resolve, reject){
        Instance.save(function (err) { //TODO: change save() to async
          if (err) return reject(err);
          ...
          Instance[association.delAccessor](function (err) {
            ...
            Extension.save(function(err, result) {
              if (err) return reject(err);
              return resolve(result);
            });
          });
  ...
  Object.defineProperty(Instance, association.delAccessor + 'Async', {
    value : function () {
      return new Promise (function(resolve, reject) {
        if (!Instance[Model.id]) {
          return reject(new ORMError("Instance not saved, cannot get extension", 'NOT_DEFINED', { model: Model.table }));
...
function autoFetchInstanceAsync(Instance, association, opts) {
  return new Promise (function(resolve, reject) {
    if (!Instance.saved()) return resolve();

    if (!opts.hasOwnProperty("autoFetchLimit") || !opts.autoFetchLimit) {
      opts.autoFetchLimit = association.autoFetchLimit;
    }

    if (opts.autoFetchLimit === 0 || (!opts.autoFetch && !association.autoFetch)) return resolve();
