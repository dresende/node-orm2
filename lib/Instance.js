...
function Instance(opts) {
		...
		handleValidations(function (err) {
			...
			for (var k in opts.data) {
				...
				if (opts.properties[k] && opts.driver.valueToProperty) {
					data[k] = opts.driver.valueToProperty(opts.data[k], opts.properties[k]);
				} else {
					data[k] = opts.data[k];
				}
			}

			if (!data.hasOwnProperty(opts.id)) {
	...
	var removeInstance = function (cb) {
		...
		var conditions = {};
		conditions[opts.id] = opts.data.id;
	...
	var saveInstanceProperty = function (key, value) {
		...
		if (opts.properties[key] && opts.driver.valueToProperty) {
